...
def process(conf,cost_p_km,map_name):
    print conf
    ...
    new_settings_file = PATH + map_name + '_conf=%s_seed=%s_costPerKM=%s.xml' % ('+'.join(map(str,conf)), str(seed), '+'.join(map(str,cost_p_km)))
    
    for elem in document.findall( 'companies/company/vehicle' ):
        if conf and cost_p_km:
            for sub_elem in elem.findall(".//*[@capacity]"):            
                cap = conf.pop(0)
                sub_elem.attrib['capacity'] = str(cap)
                break
            for sub_elem in elem.findall(".//*[@cost-per-km]"):
                cost = cost_p_km.pop(0)
                sub_elem.attrib['cost-per-km'] = str(cost)
                break
        else:
            break
...
if __name__ == '__main__':
    ...
    vehicle_size_confs = list([ [choice(sizes)]*3 for x in xrange(10)])
    vehicle_cost_per_km = list([ [choice(sizes),choice(sizes),choice(sizes)] for x in xrange(10)])
    confs = list(product(vehicle_size_confs,vehicle_cost_per_km,maps))
    confs = [(list(x[0]),list(x[1]),str(x[2])) for x in confs]
